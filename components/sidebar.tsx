...
interface SidebarProps {
  ...
  arrayState: ArrayState & {
    ...
    executeInsertElement: (position: number, value: number) => void;
    executeDeleteElement: (position: number) => void;
    executeUpdateElement: (position: number, value: number) => void;
    reset: () => void;
  };
  ...
  const basicOperations = [
    {
      ...
      name: "Create Array",
      icon: Plus,
      description: "Initialize array with custom size",
    },
    {
      id: "insert",
      name: "Insert Element",
      icon: Plus,
      description: "Add element at specific index",
    },
    {
      id: "update",
      name: "Update Element",
      icon: Edit,
      description: "Modify element value",
    },
    {
      id: "delete",
      name: "Delete Element",
    ...
    },
    {
      ...
      name: "Display Array",
      icon: Eye,
      description: "Show current array state",
    },
  ];
  ...
  const handleOperationClick = (operationId: string) => {
    ...
    switch (operationId) {
      ...
      case "create":
        ...
        break;
      case "insert":
        // Use random position and value for demo
        const randomPos = Math.floor(
          Math.random() * (arrayState.elements.length + 1)
        );
        const randomValue = Math.floor(Math.random() * 100) + 1;
        arrayState.executeInsertElement(randomPos, randomValue);
        break;
      case "update":
        // Use random position and value for demo
        const randomUpdatePos = Math.floor(
          Math.random() * arrayState.elements.length
        );
        const randomUpdateValue = Math.floor(Math.random() * 100) + 1;
        arrayState.executeUpdateElement(randomUpdatePos, randomUpdateValue);
        break;
      case "delete":
        // Use random position for demo
        const randomDeletePos = Math.floor(
          Math.random() * arrayState.elements.length
        );
        arrayState.executeDeleteElement(randomDeletePos);
        break;
      case "display":
                ...
                <CardContent className="space-y-3">
                  <div>
                    ...
                    <Input
                      ...
                      value={arraySize}
                      onChange={(e) =>
                        setArraySize(Number.parseInt(e.target.value))
                      }
              ...
              <Card className="bg-slate-700 border-slate-600">
                ...
                <CardContent>
                  ...
                  <div className="text-xs text-slate-400 mt-2">
                    Size: {arrayState.elements.length} elements
                  </div>
                </CardContent>
              </Card>
            </div>
